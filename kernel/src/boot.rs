...
impl BootPageTable {
    ...
    pub fn init(&mut self) {
        ...
        let pstart = _start as usize;
        let vstart = ENTRY_VADDR;
        let offset = vstart - pstart;
        // 确保虚实地址在 1 GiB 内对齐
        assert!(offset.trailing_zeros() >= 30);
        // 映射跳板页
        let base = VAddr::<Sv39>::new(pstart).floor().index_in(Sv39::MAX_LEVEL);
    ...
    pub unsafe fn launch(&self) -> usize {
        ...
        // 跳到高页面对应位置
        Self::jump_higher(ENTRY_VADDR - _start as usize);
        // 设置内核可访问用户页
